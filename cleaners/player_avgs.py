...
# get averages grouped by player keeping the player name, team, and position
player_avgs = (
    box_scores.groupby(["Player", "Team", "Position"]).mean().reset_index().round(2)
)

# order by position then fpts
player_avgs = player_avgs.sort_values(by=["Fpts"], ascending=[False])

# output to csv
player_avgs.to_csv("player_avgs.csv", index=False)
